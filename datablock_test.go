...
func TestDataBlockDecode(t *testing.T) {
	...
	dataSet := []DataBlockTest{
		...
		{
			name:        "CAT048 only category field",
			input:       "30",
			err:         io.EOF,
			nbOfRecords: 0,
			unRead:      0,
		...
		{
			name:        "CAT048 in end a byte 0xf5 has been removed",
	...
	for _, row := range dataSet {
		...
		// Assert
		if err != row.err {
			t.Errorf("FAIL: %s error: %s; Expected: %v", row.name, err, row.err)
		} else {
			t.Logf("SUCCESS: error: %v; Expected: %v", err, row.err)
		}
		if unRead != row.unRead {
			t.Errorf("FAIL: %s unRead = %v; Expected: %v", row.name, unRead, row.unRead)
		} else {
			t.Logf("SUCCESS: unRead = %v; Expected: %v", unRead, row.unRead)
		}
		if row.nbOfRecords != len(dataB.Records) {
			t.Errorf("FAIL: %s nbOfRecords = %v; Expected: %v", row.name, len(dataB.Records), row.nbOfRecords)
		} else {
